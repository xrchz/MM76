open HolKernel boolLib bossLib boolSimps Parse finite_mapTheory termTheory pred_setTheory listTheory substitutionTheory triangular_substitutionTheory walkTheory walkstarTheory lcsymtacs
...
`wfs s ==> !t.(SAPPLY (collapse s) t = walkstar s t)`,
STRIP_TAC THEN HO_MATCH_MP_TAC walkstar_ind THEN
SRW_TAC [][] THEN
Cases_on `t` THEN
fsrw_tac [DNF_ss][LIST_EQ_REWRITE,EL_MAP,MEM_EL] THEN
SRW_TAC [][collapse_def,FLOOKUP_DEF,FUN_FMAP_DEF] THEN
qmatch_assum_rename_tac `n ∉ FDOM s` [] >>
Cases_on `vwalk s n` THEN FULL_SIMP_TAC (srw_ss()) [] THEN
PROVE_TAC [NOT_FDOM_vwalk,term_11,term_distinct]);
...
SRW_TAC [][] THEN
Cases_on `t` >>
fsrw_tac [DNF_ss][LIST_EQ_REWRITE,EL_MAP,MEM_EL] >>
...
`wfs s ==> (idempotent s <=> (walkstar s = SAPPLY s))`,
...
REVERSE (Cases_on `x IN FDOM s`) THEN1
  METIS_TAC [NOT_FDOM_FAPPLY_FEMPTY] THEN
METIS_TAC [collapse_APPLY_eq_walkstar,SAPPLY_FAPPLY]);

val SAPPLY_walkstar = Q.store_thm(
"SAPPLY_walkstar",
`wfs s ⇒ ∀t. SAPPLY s (walk* s t) = (walk* s t)`,
STRIP_TAC THEN HO_MATCH_MP_TAC apply_ts_ind THEN
SRW_TAC [DNF_ss][LIST_EQ_REWRITE,EL_MAP,MEM_EL,apply_ts_thm] THEN
Cases_on `FLOOKUP s v` THEN SRW_TAC [][] );

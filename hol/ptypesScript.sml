...
`av.tail ≠ lv.first` by (
  spose_not_then strip_assume_tac >>
  qsuff_tac `ls = h::ls` >- srw_tac [][] >>
  match_mp_tac (MP_CANON (GEN_ALL list_of_AuxList_unique)) >>
  map_every qexists_tac [`s`,`lv.last`,`emb`,`av.tail`] >>
  srw_tac [][] >>
  srw_tac [][Once list_of_AuxList_cases] >>
  PROVE_TAC [] ) >>
...
srw_tac [][] >>
`ptr_to_num l ≠ 0` by (
  fsrw_tac [][lookup_succeeds,FLOOKUP_DEF,wfstate_def] >>
  PROVE_TAC [] ) >>
`tailR s.store lv.last (ptr_to_num lv.last) (ptr_to_num av.tail)`
  by PROVE_TAC [list_of_AuxList_imp_tailR] >>
`ptr_to_num l ∈ FDOM s.store` by fsrw_tac [][lookup_succeeds,FLOOKUP_DEF] >>
`s.cell_type (ptr_to_num l) = List_type emb.type` by fsrw_tac [][lookup_succeeds] >>
`s.cell_type (ptr_to_num lv.first) = AuxList_type emb.type` by fsrw_tac [][lookup_succeeds] >>
...
`s.cell_type (ptr_to_num av.tail) = AuxList_type emb.type` by (
  fsrw_tac [][wfstate_def] >>
  imp_res_tac typed_state_def >>
  pop_assum mp_tac >>
  fsrw_tac [][typed_cell_def,lookup_succeeds] >>
  Cases_on `v` >> fsrw_tac [][] >>
  qmatch_assum_rename_tac `project_AuxList w = SOME av` [] >>
  Cases_on `w` >> fsrw_tac [][] >>
  srw_tac [][Once has_type_cases] >>
  fsrw_tac [][FLOOKUP_UPDATE] >>
  srw_tac [][] >> fsrw_tac [][] >>
  fsrw_tac [][DOMSUB_FLOOKUP_THM,FLOOKUP_UPDATE] >>
  srw_tac [][] >> fsrw_tac [][] >>
  fsrw_tac [][typed_cell_def] >> srw_tac [][] >>
  qpat_assum `ptr_to_num l ≠ 0` assume_tac >> fsrw_tac [][] >>
  qpat_assum `FLOOKUP s.store n = SOME (AuxList_value X Y)` assume_tac >>
  fsrw_tac [][] >>
...
`ptr_to_num lv.last ≠ 0 ⇒ (s.cell_type (ptr_to_num lv.last) = AuxList_type emb.type)` by (
  strip_tac >>
  match_mp_tac (MP_CANON (GEN_ALL list_of_AuxList_tailR_type)) >>
  map_every qexists_tac [`lv.last`,`av.tail`,`ls`] >>
  fsrw_tac [][] ) >>
`ptr_to_num lv.last ≠ ptr_to_num l` by (
  spose_not_then (strip_assume_tac o SYM) >>
  fsrw_tac [][lookup_succeeds,wfstate_def] >>
  qpat_assum `ptr_to_num lv.last ≠ 0` assume_tac >>
  fsrw_tac [][] ) >>
`s'' = s' with store updated_by (λs. s \\ ptr_to_num lv.first)` by (
  Cases_on `lv.first` >> fsrw_tac [][] ) >>
`s'.cell_type = s.cell_type` by srw_tac [][] >>
`s''.cell_type = s.cell_type` by srw_tac [][] >>
qabbrev_tac `m = ptr_to_num lv.first` >>
qabbrev_tac `s''2 = (s' with <|store updated_by (combin$C $\\ m);
                               cell_type updated_by (m =+ (s.cell_type m))|>)` >>
`s''2 = s''` by (
  srw_tac [][Abbr`s''2`,state_component_equality,FUN_EQ_THM,APPLY_UPDATE_THM] >>
  srw_tac [][] >> metis_tac []) >>
qsuff_tac `list_of_AuxList emb s''2 lv.last av.tail ls` >- srw_tac [][] >>
qunabbrev_tac `s''2` >>
match_mp_tac (MP_CANON list_of_AuxList_remove_unreachable) >>
`~headR s.store lv.last (ptr_to_num l) (ptr_to_num av.tail)` by (
  spose_not_then strip_assume_tac >>
  `s.cell_type (ptr_to_num l) = emb.type` by (
    match_mp_tac (MP_CANON (GEN_ALL list_of_AuxList_headR_type)) >>
    map_every qexists_tac [`lv.last`,`av.tail`,`ls`] >>
    fsrw_tac [][] ) >>
  PROVE_TAC [type_inductive] ) >>
`~tailR s.store lv.last (ptr_to_num l) (ptr_to_num av.tail)` by (
  spose_not_then strip_assume_tac >>
  `s.cell_type (ptr_to_num l) = AuxList_type emb.type` by (
    match_mp_tac (MP_CANON (GEN_ALL list_of_AuxList_tailR_type)) >>
    map_every qexists_tac [`lv.last`,`av.tail`,`ls`] >>
    fsrw_tac [][] ) >>
  fsrw_tac [][] ) >>
conj_asm1_tac >- (
  qabbrev_tac  `s'2 = (s with <|store updated_by (ptr_to_num l =+ inject_List (List av.tail lv.last));
                                cell_type updated_by (ptr_to_num l =+ s.cell_type (ptr_to_num l))|>)` >>
  `s'2 = s'` by (
    srw_tac [][Abbr`s'2`,state_component_equality,FUN_EQ_THM,APPLY_UPDATE_THM] >>
    srw_tac [][] >> metis_tac [] ) >>
  qsuff_tac `list_of_AuxList emb s'2 lv.last av.tail ls` >- srw_tac [][] >>
  ...
  srw_tac [][] ) >>
`m ≠ 0` by (
  fsrw_tac [][Abbr`m`,lookup_succeeds,FLOOKUP_DEF,wfstate_def] >>
  PROVE_TAC [] ) >>
conj_asm2_tac >- (
  spose_not_then strip_assume_tac >>
  qsuff_tac `s.cell_type m = emb.type` >- metis_tac [type_inductive] >>
  match_mp_tac (MP_CANON (GEN_ALL list_of_AuxList_headR_type)) >>
  map_every qexists_tac [`lv.last`,`av.tail`,`ls`] >>
  qpat_assum `~tailR s.store lv.last (ptr_to_num l) X` assume_tac >>
  srw_tac [][] >>
  fsrw_tac [][tailR_assign_unreachable] >>
  qpat_assum `~tailR s.store lv.last m X` assume_tac >>
  fsrw_tac [][headR_assign_unreachable] ) >>
qsuff_tac `~tailR s.store lv.last m (ptr_to_num av.tail)` >-
  (srw_tac [][] >> fsrw_tac [][tailR_assign_unreachable]) >>
qpat_assum `s' = X` (K ALL_TAC) >>
qpat_assum `s'' = X` (K ALL_TAC) >>
qpat_assum `X = s''` (K ALL_TAC) >>
srw_tac [][Abbr`m`] >>
`tailR s.store lv.last (ptr_to_num av.tail) (ptr_to_num lv.first)` by (
  srw_tac [][Once RTC_CASES2,tailR1_def] >>
  fsrw_tac [][lookup_succeeds] >>
  qmatch_assum_rename_tac `FLOOKUP s.store (ptr_to_num lv.first) = SOME w` [] >>
  Cases_on `w` >> fsrw_tac [][] >>
  srw_tac [][] >>
  Cases_on `v` >> fsrw_tac [][] >>
  srw_tac [][] >> fsrw_tac [][] ) >>
spose_not_then strip_assume_tac >>
`ptr_to_num av.tail = ptr_to_num lv.first` by
  metis_tac [list_of_AuxList_imp_tailR_antisymmetric] >>
fsrw_tac [][ptr_equality]);

val dispose_succeeds = Q.store_thm(
"dispose_succeeds",
...
val AuxList_cells_def = Define`
  AuxList_cells s last l = {m | tailR s last m l}`;

val List_cells_def = Define`
  List_cells s l = AuxList_cells s l.last (ptr_to_num l.first)`;
...
`wfstate s ∧ is_embed emb ∧
 ...
 DISJOINT
   (List_cells s.store (FST (THE (lookup emb l1 s))))
   (List_cells s.store (FST (THE (lookup emb l2 s)))) ⇒
 ∃s'. (AppendLists emb l1 l2 s = SOME (l1,s')) ∧
      list_of_List emb s' l1 (ls1 ++ ls2)`,
srw_tac [][list_of_List_def] >>
pop_assum mp_tac >> srw_tac [][] >>
imp_res_tac lookup_state >> srw_tac [][] >>
qmatch_assum_rename_tac `lookup emb l2 (SND z1) = SOME z2` [] >>
Cases_on `z1` >> Cases_on `z2` >> fsrw_tac [][] >> srw_tac [][] >>
qmatch_assum_rename_tac `lookup emb l1 s = SOME (lv1,s)` [] >>
qmatch_assum_rename_tac `lookup emb l2 s = SOME (lv2,s)` [] >>
`ptr_to_num l1 ∈ FDOM s.store` by fsrw_tac [][lookup_succeeds,FLOOKUP_DEF] >>
`ptr_to_num l2 ∈ FDOM s.store` by fsrw_tac [][lookup_succeeds,FLOOKUP_DEF] >>
`s.cell_type (ptr_to_num l1) = List_type emb.type` by fsrw_tac [][lookup_succeeds] >>
`s.cell_type (ptr_to_num l2) = List_type emb.type` by fsrw_tac [][lookup_succeeds] >>
`(ptr_to_num lv2.first ≠ 0 ⇒ (s.cell_type (ptr_to_num lv2.first) = AuxList_type emb.type)) ∧
 (ptr_to_num lv2.last ≠ 0 ⇒ (s.cell_type (ptr_to_num lv2.last) = AuxList_type emb.type))` by (
  fsrw_tac [][wfstate_def] >>
  imp_res_tac typed_state_def >>
  ...
  fsrw_tac [][] >> srw_tac [][] >>
  fsrw_tac [][] ) >>
`(ptr_to_num lv1.first ≠ 0 ⇒ (s.cell_type (ptr_to_num lv1.first) = AuxList_type emb.type)) ∧
 (ptr_to_num lv1.last ≠ 0 ⇒ (s.cell_type (ptr_to_num lv1.last) = AuxList_type emb.type))` by (
  fsrw_tac [][wfstate_def] >>
  imp_res_tac typed_state_def >>
  ntac 2 (pop_assum mp_tac) >>
  fsrw_tac [][typed_cell_def,lookup_succeeds] >>
  ...
  fsrw_tac [][] >> srw_tac [][] >>
  fsrw_tac [][]) >>
`ptr_to_num l1 ≠ 0` by PROVE_TAC [wfstate_def] >>
`ptr_to_num l2 ≠ 0` by PROVE_TAC [wfstate_def] >>
`ptr_to_num l1 ≠ ptr_to_num lv1.first ∧
 ptr_to_num l1 ≠ ptr_to_num lv1.last ∧
...
fsrw_tac [][List_cells_def,AuxList_cells_def,IN_DISJOINT] >>
...
reverse (srw_tac [DNF_ss][AppendLists_def,UNCURRY]) >- (
  ...
  srw_tac [][] >>
  (dispose_succeeds |> Q.ISPECL_THEN [`l2`,`s'`] strip_assume_tac) >>
  srw_tac [][] >>
  (lookup_dispose |> Q.GEN `p1` |> Q.ISPEC `l2:'a List ptr`
                  |> Q.GEN `p2` |> Q.ISPEC `l1:'a List ptr`
                  |> imp_res_tac) >>
  fsrw_tac [][ptr_equality] >>
  pop_assum (K ALL_TAC) >>
  (lookup_dispose |> Q.GEN `p1` |> Q.ISPEC `lv2.first`
                  |> Q.GEN `p2` |> Q.ISPEC `l1:'a List ptr`
                  |> imp_res_tac) >>
  fsrw_tac [][ptr_equality] >>
  pop_assum (K ALL_TAC) >>
  ...
  `s'' = s' with <|store updated_by combin$C $\\ m; cell_type updated_by (m =+ s'.cell_type m)|>` by (
    Cases_on `l2` >> fsrw_tac [][] >>
    srw_tac [][state_component_equality,FUN_EQ_THM,APPLY_UPDATE_THM] ) >>
  srw_tac [][] >>
  match_mp_tac (MP_CANON list_of_AuxList_remove_unreachable) >>
  qunabbrev_tac `m` >>
  qabbrev_tac `m = ptr_to_num lv2.first` >>
  `s' = s with <|store updated_by combin$C $\\ m; cell_type updated_by (m =+ s.cell_type m)|>` by (
    Cases_on `lv2.first` >> fsrw_tac [][] >>
    srw_tac [][state_component_equality,FUN_EQ_THM,APPLY_UPDATE_THM] ) >>
  `~tailR s.store lv1.last m (ptr_to_num lv1.first)` by metis_tac [RTC_REFL] >>
  fsrw_tac [][headR_remove_unreachable,tailR_remove_unreachable] >>
  reverse conj_tac >- (
    conj_tac >> spose_not_then strip_assume_tac >- (
      `s.cell_type (ptr_to_num l2) = emb.type` by (
        match_mp_tac (MP_CANON (GEN_ALL list_of_AuxList_headR_type)) >>
        map_every qexists_tac [`lv1.last`,`lv1.first`,`ls1`] >>
        srw_tac [][] >>
        first_x_assum match_mp_tac >>
        fsrw_tac [][headR_def,FLOOKUP_DEF,Once RTC_CASES2,tailR1_def] >>
        PROVE_TAC [wfstate_def] ) >>
      metis_tac [type_inductive] ) >>
    `s.cell_type (ptr_to_num l2) = AuxList_type emb.type` by (
      match_mp_tac (MP_CANON (GEN_ALL list_of_AuxList_tailR_type)) >>
      map_every qexists_tac [`lv1.last`,`lv1.first`,`ls1`] >>
      srw_tac [][] >>
      first_x_assum match_mp_tac >>
      fsrw_tac [][FLOOKUP_DEF,Once RTC_CASES2,tailR1_def] >>
      PROVE_TAC [wfstate_def] ) >>
    fsrw_tac [][] ) >>
  Cases_on `m=0` >- (
    qmatch_abbrev_tac `list_of_AuxList emb ss lv1.last lv1.first ls1` >>
    `ss = s` by (
      srw_tac [][state_component_equality,Abbr`ss`,FUN_EQ_THM,APPLY_UPDATE_THM] >>
  ...
  `s.cell_type m = emb.type` by (
    ...
    srw_tac [][] >>
    first_x_assum match_mp_tac >>
    fsrw_tac [][Abbr`m`,headR_def,FLOOKUP_DEF,Once RTC_CASES2,tailR1_def] >>
    PROVE_TAC [wfstate_def] ) >>
  metis_tac [type_inductive] ) >>
`?h2 t2. lookup emb lv2.first s = SOME (AuxList h2 t2,s)` by (
  fsrw_tac [][Once list_of_AuxList_cases] >>
  qmatch_assum_rename_tac `lookup emb lv2.first s = SOME p` [] >>
  Cases_on `p` >> srw_tac [][] >>
...
`?s'. assign emb lv1.last (AuxList h2 t2) s = SOME ((),s')` by (
  `ptr_to_num lv1.last ∈ FDOM s.store` by (
    ...
    `ptr_to_num lv1.last = 0` by (
      match_mp_tac (GEN_ALL cell_reach_typed_state_unbound_eq_0) >>
      map_every qexists_tac [`s`,`ptr_to_num l1`] >>
      fsrw_tac [][wfstate_def] >>
      match_mp_tac RTC_SUBSET >>
      srw_tac [][cell_reach1_def] >>
      fsrw_tac [][lookup_succeeds] >>
      Cases_on `v` >> fsrw_tac [][] >> srw_tac [][reach1_cases] ) >>
    fsrw_tac [][GSYM ptr_equality] ) >>
  Cases_on `lv1.last` >> fsrw_tac [][ptr_equality,ITSELF_UNIQUE] ) >>
srw_tac [][] >>
`?s''. assign emb l1 (List lv1.first lv2.last) s' = SOME ((),s'')` by (
  imp_res_tac assign_FDOM >>
  imp_res_tac assign_cell_type >>
  Cases_on `l1` >> fsrw_tac [][APPLY_UPDATE_THM] ) >>
srw_tac [][] >>
`?s'''. dispose lv2.first s'' = SOME ((),s''')` by
  MATCH_ACCEPT_TAC dispose_succeeds >>
`?s''''. dispose l2 s''' = SOME ((),s'''')` by
  MATCH_ACCEPT_TAC dispose_succeeds >>
srw_tac [][] >>
(lookup_dispose
 |> Q.GEN `p2` |> Q.ISPEC `l1:'a ptr`
 |> Q.GEN `x` |> Q.SPEC `(),s''''`
 |> MATCH_MP |> (fn f => first_assum (assume_tac o Q.GEN`emb` o f))) >>
fsrw_tac [][] >> pop_assum (K ALL_TAC) >>
srw_tac [DNF_ss][ptr_equality,EXISTS_PROD] >>
(lookup_dispose
 |> Q.GEN `p1` |> Q.ISPEC `lv2.first`
 |> Q.GEN `p2` |> Q.ISPEC `l1:'a List ptr`
 |> Q.GEN `x` |> Q.SPEC `(),s'''`
 |> MATCH_MP |> (fn f => first_assum (assume_tac o Q.GEN`emb` o f))) >>
fsrw_tac [][] >> pop_assum (K ALL_TAC) >>
srw_tac [DNF_ss][ptr_equality,EXISTS_PROD] >>
`lookup emb l1 (SND ((),s'')) = SOME (List lv1.first lv2.last, (SND ((),s'')))` by (
...
`list_of_AuxList emb s' lv2.last lv1.first (ls1 ++ ls2)` by (
  ...
  conj_tac >- (
    ...
    fsrw_tac [][type_inductive] ) >>
  conj_tac >- (
    srw_tac [][Abbr`m`,tailR_assign_last] >>
    metis_tac [list_of_AuxList_imp_tailR] ) >>
  ...
  conj_tac >- (
    Cases_on `m=0` >- fsrw_tac [][GSYM ptr_equality] >> fsrw_tac [][] >>
    spose_not_then strip_assume_tac >>
    qsuff_tac `s.cell_type m = emb.type` >- PROVE_TAC [type_inductive] >>
    match_mp_tac (MP_CANON (GEN_ALL list_of_AuxList_headR_type)) >>
    map_every qexists_tac [`lv2.last`,`t2`,`tl`] >>
    srw_tac [][] >>
    `ptr_to_num lv2.first ∈ FDOM s.store` by fsrw_tac [][lookup_succeeds,FLOOKUP_DEF] >>
    fsrw_tac [][wfstate_def] >> imp_res_tac typed_state_def >>
    qpat_assum `typed_cell s {} (ptr_to_num lv2.first)` mp_tac >>
    fsrw_tac [][lookup_succeeds,typed_cell_def] >>
    srw_tac [][Once has_type_cases] >>
    fsrw_tac [][] >> srw_tac [][] >>
    first_x_assum match_mp_tac >>
    spose_not_then strip_assume_tac >>
    fsrw_tac [][headR_def] >>
    qpat_assum `tailR s.store lv2.last a 0` mp_tac >>
    srw_tac [][Once RTC_CASES2,tailR1_def,FLOOKUP_DEF] >>
    fsrw_tac [][FLOOKUP_DEF] >>
    metis_tac [] ) >>
  spose_not_then strip_assume_tac >>
  qsuff_tac `tailR s.store lv2.last m (ptr_to_num lv2.first)` >-
    metis_tac [list_of_AuxList_imp_tailR] >>
  srw_tac [][Once RTC_CASES2,tailR1_def,ptr_equality] >>
  fsrw_tac [DNF_ss][lookup_succeeds] >>
  qmatch_assum_rename_tac `project_AuxList al = SOME (AuxList h2 t2)` [] >>
  Cases_on `al` >> fsrw_tac [][] >> srw_tac [][] >> fsrw_tac [][] ) >>

...
val _ = type_abbrev("type_of", ``:'a itself -> type``);
val _ = Hol_datatype`embed = <| type_of : 'a type_of ; inject : 'a inject ; project : 'a project |>`;
...
val _ = overload_on("embed_Variable",``embed (K Variable_type) inject_Variable project_Variable : Variable embed``);
...
val _ = overload_on("embed_SetOfVariables",``embed (K SetOfVariables_type) inject_SetOfVariables project_SetOfVariables : SetOfVariables embed``);
...
val project_Term_def = Define`
  ...
  (project_Term _ = NONE)`;
val _ = overload_on("embed_Term",``embed (K Term_type) inject_Term project_Term : Term embed``);
...
val _ = overload_on("embed_Multiequation",``embed (K Multiequation_type) inject_Multiequation project_Multiequation : Multiequation embed``);
...
val _ = overload_on("embed_TempMultiequation",``embed (K TempMultiequation_type) inject_TempMultiequation project_TempMultiequation : TempMultiequation embed``);
...
val _ = overload_on("embed_System",``embed (K System_type) inject_System project_System : System embed``);
...
val _ = overload_on("embed_AuxList", ``λemb. embed (K (AuxList_type (emb.type_of (:'a)))) inject_AuxList project_AuxList : 'a AuxList embed``);
...
val project_List_def = Define`
  ...
  (project_List _ = NONE)`;
val _ = overload_on("embed_List", ``λemb. embed (K (List_type (emb.type_of (:'a)))) inject_List project_List : 'a List embed``);
